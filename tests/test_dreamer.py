...
def test_replay_sample_shapes():
    ...
    for i in range(120):
        rb.add(np.random.randn(4).astype("float32"), np.eye(2)[i % 2].astype("float32"),
               float(i), i % 30 == 0, False)
...
def test_full_train_step(is_image, is_discrete):
    ...
    for i in range(80):
        ...
        rb.add(obs, np.random.randn(cfg.action_dim).astype("float32"),
               float(np.random.randn()), i % 20 == 0, False)
    batch = rb.sample(2, 6, torch.device("cpu"))
